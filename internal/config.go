...
type Config struct {
	...
	AuthHosts              []string             `long:"auth-hosts" env:"AUTH_HOST" description:"Single host to use when returning from 3rd party auth"`
...
func (c *Config) Validate() {
	...
	for _, host := range c.AuthHosts {
		match, _ := matchCookieDomains(host)
		if !match {
			log.Fatalf("auth host%s ")
		}
	}
